...
export function SectionCards({ totalRevenue, totalGuests, eventCount, averageSellThrough }: SectionCardsProps) {
  return (
    <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-0 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
      ...
      <Card className="@container/card">
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            Tickets sold per capacity <IconTrendingUp className="size-4" />
          </div>
          <div className="text-muted-foreground">Measure of demand.</div>
        </CardFooter>
      </Card>
    </div>
  );
