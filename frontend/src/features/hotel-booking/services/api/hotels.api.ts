...
export const hotelApi = {
  ...
  getMyHotels: async (): Promise<Hotel[]> => {
    try {
      ...
      console.log("🏨 Calling endpoint: /api/hotels/my-hotels");
      ...
      if (response.data.data && response.data.data.length > 0) {
        ...
        try {
          const allHotelsResponse = await api.get("/api/hotels");
      ...
      try {
        ...
        const fallbackResponse = await api.get("/api/hotels");
  ...
  createHotel: async (data: CreateHotelRequest & { imageFile?: File }): Promise<Hotel> => {
    ...
    if (data.imageFile) {
      try {
        ...
        imagePath = await uploadToResourceBucket(data.imageFile, "hotels", "NadPerz");
  ...
  updateHotel: async (id: string, data: UpdateHotelRequest & { imageFile?: File }): Promise<Hotel> => {
    ...
    if (data.imageFile) {
      try {
        ...
        const imagePath = await uploadToResourceBucket(data.imageFile, "hotels", "NadPerz");
    ...
    const response = await api.put(`/api/hotels/${id}`, cleanUpdateData);
  ...
  searchHotelsByLocation: async (city: string, state?: string, country?: string): Promise<Hotel[]> => {
    try {
      ...
      const response = await api.get(`/api/hotels/search/location?${params.toString()}`);
  ...
  getHotelsWithFilters: async (filters: { city?: string; state?: string; country?: string }): Promise<Hotel[]> => {
    try {
      const params = new URLSearchParams();
